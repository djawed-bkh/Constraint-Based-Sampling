...
public class Evaluation {
    ...
    public void expeVolumesurFrequence(Database db,int seed, int nbmotifs, String outputpathprefix, String outputpathsuffix){
        ...
        FIPS fips = new FIPS(db, seed);
        HFIPS hips = new HFIPS(db, seed);
        RandomIPSampling uniform = new RandomIPSampling(db, seed);
...
/*            // add the frequencies of the patterns
    ...
    public void calculateAndPrintAveragesAndStdDev(ArrayList<IP> ipList, int groupSize, String filePath) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(filePath))) {
            ...
            for (int i = 0; i < ipList.size(); i += groupSize) {
                ...
                // Écriture des valeurs dans le fichier CSV
                //System.out.printf("Groupe %d -> Moyenne_Fréquence: %.5f, Écart_Type_Fréquence: %.5f, Moyenne_Volume: %s, Écart_Type_Volume: %s%n",
    ...
    // Méthode pour calculer et afficher la moyenne des fréquences par groupe
//    private  void calculateAndPrintAveragesAndStdDev(ArrayList<IP> ipList, int groupSize) {
//
//
//        for (int i = 0; i < ipList.size(); i += groupSize) {
//            int end = Math.min(i + groupSize, ipList.size());
...
//            double stdDevFrequency = calculateStandardDeviationFrequency(group, averageFrequency);
//            double stdDevVolume = calculateStandardDeviationVolume(group,averageVolume);
//            System.out.printf("Groupe [%d - %d] : Moyenne Fréquence = %.2f, Écart type Fréquence = %.2f, Moyenne Volume = %.2f, Écart type Volume = %.2f%n",
//                    i, end - 1, averageFrequency, stdDevFrequency, averageVolume.doubleValue(), stdDevVolume);
//        }
//    }
    ...
    public void volumeAndFrequencyUniform(int seed, int k, int rep, Database db){
        RandomIPSampling fips = new RandomIPSampling(db, seed);
    ...
    /*public void evaluationpreprocessingTime(CFIPS sampling, ArrayList<Constraint> query){
        long startTime = System.currentTimeMillis();
        sampling.ProcessObjectsWeights(query);
        System.out.println(String.format("Pre-processing cpu time: %.3f s",
                (System.currentTimeMillis() - startTime) / 1000.0));

    }*/

    public void evaluationpreprocessingTime(CHFIPS sampling, ArrayList<ConstraintNewList.Constraint> query){
        long startTime = System.currentTimeMillis();
        sampling.ProcessObjectsWeights(query);
        System.out.println(String.format("Pre-processing cpu time: %.3f s",
                (System.currentTimeMillis() - startTime) / 1000.0));
    ...
    public void evaluateRandomNumberOfDraws(int seed,Database db, String outputfile, int requiredNumberOfPatterns, int repetitions, ArrayList<ConstraintNewList.Constraint> query) {

        //DB: Checked

        ArrayList<HashMap<String, Object>> resultsList = new ArrayList<>();

        for(int c = 1; c<= query.size(); c++ ) {    // constraints form 1 to n
            ArrayList<ConstraintNewList.Constraint> tempConstraint = new ArrayList<>(query.subList(0, c));
            RandomIPSampling random = new RandomIPSampling(db, seed);
            double GlobalmeanFrequency =0.0;
            double meanDraws = 0.0;
            boolean attainedTimeout = false;
            long startTime = System.currentTimeMillis();
            long timeout = 120_000; // 2 minutes
            long MeanCpuTime = 0;

            for (int iteration = 0; iteration < repetitions; iteration++) {
                // repeat to have a mean
                int patternCount = 0;
                int totalDraws = 0;
                long totalCpuTime = 0;
                double meanFrequency= 0.0;
                while (patternCount < requiredNumberOfPatterns) {
                    // Timeout check
                    if (System.currentTimeMillis() - startTime > timeout) {
                        System.out.println("Timeout occurred (2 minutes) with " + totalDraws + " iterations.");
                        attainedTimeout = true;
                        break;
                    }
                    ...
                    if (ConstraintsSatisfiednewFormat(ip, tempConstraint)) {
                        patternCount++;
                        meanFrequency += ip.getFrequency();
                    }
                }   //end while
                if (attainedTimeout) {
                    break;
                }
                GlobalmeanFrequency += meanFrequency /requiredNumberOfPatterns;
                MeanCpuTime += totalCpuTime;
                meanDraws += totalDraws;
            }//end repetition

            if(!attainedTimeout){
                double resultNbDraws = meanDraws / repetitions;
                double resultCPUTime = ((double) MeanCpuTime / repetitions) / 1000.0;
                HashMap<String, Object> resultDict = new HashMap<>();
                resultDict.put("constraintNumber", c);
                resultDict.put("query", tempConstraint.toString());
                resultDict.put("resultNbDraws", resultNbDraws);
                resultDict.put("resultCPUTime", resultCPUTime);
                resultDict.put("resultFrequency", GlobalmeanFrequency /repetitions);

                // Add the dictionary to the list
                resultsList.add(resultDict);
            }else{
                HashMap<String, Object> resultDict = new HashMap<>();
                resultDict.put("constraintNumber", c);
                resultDict.put("query", tempConstraint.toString());
                resultDict.put("resultNbDraws", "TO");
                resultDict.put("resultCPUTime", "TO");
                resultDict.put("resultFrequency", "TO");
        ...
        writeResults(outputfile, resultsList);
    }

    public void evaluateHFIPSNumberOfDraws(int seed,Database db, String outputfile, int numberOfPatterns, int repetitions,ArrayList<ConstraintNewList.Constraint> query) {
        //DB: checked
        ...
        for(int c = 1; c<= query.size(); c++ ) {    // constraints form 1 to n
            ...
            for (int iteration = 0; iteration < repetitions; iteration++) {
                ...
                while (patternCount < numberOfPatterns) {

                    // Timeout check
                    if (System.currentTimeMillis() - startTime > timeout) {
                        System.out.println("Timeout occurred (2 minutes) after " + totalDraws + " iterations.");
                        attainedTimeout = true;
                        break;
                    }
                ...
                GlobalmeanFrequency += meanFrequency /numberOfPatterns;
                MeanCpuTime += totalCpuTime;
                meanDraws += totalDraws;
            }
            ...
            if(!attainedTimeout){
                double resultNbDraws = meanDraws / repetitions;
                double resultCPUTime = ((double) MeanCpuTime / repetitions) / 1000.0;   // on divise par 1000 pour passer en secondes
                HashMap<String, Object> resultDict = new HashMap<>();
                resultDict.put("constraintNumber", c);
                resultDict.put("query", tempConstraint.toString());
                resultDict.put("resultNbDraws", resultNbDraws);
    ...
    public void evaluateFIPSNumberOfDraws(int seed,Database db, String outputfile, int numberOfPatterns, int repetitions,ArrayList<ConstraintNewList.Constraint> query) {
        //DB: checked
    ...
    public void evaluateOptimizedCFIPSNumberOfDraws(int seed, Database db, String outputfile, int numberOfPatterns, int repetitions, ArrayList<ConstraintNewList.Constraint> query) {

        ArrayList<HashMap<String, Object>> resultsList = new ArrayList<>();
        for(int c = 1; c<= query.size(); c++ ) {    // constraints form 1 to n
            boolean attainedTimeout = false;
            ArrayList<ConstraintNewList.Constraint> tempConstraint = new ArrayList<>(query.subList(0, c));
            OptimizedCFips cfips = new OptimizedCFips(db, seed, tempConstraint);
            double GlobalmeanFrequency =0.0;
            double meanDraws = 0.0;
            long startTime = System.currentTimeMillis();
            long timeout = 120_000; // 2 minutes
            ...
            for (int iteration = 0; iteration < repetitions; iteration++) {
                ...
                double meanFrequency= 0.0;
                while (patternCount < numberOfPatterns) {
                    if(totalDraws > numberOfPatterns){
                        System.out.println("PROBLEM");
                    }
                    // Timeout check
                    if (System.currentTimeMillis() - startTime > timeout) {
                        System.out.println("Timeout occurred after (2 minutes) " + totalDraws + " iterations.");
                        attainedTimeout= true;
                        break;
                    }
                    ...
                    if (ConstraintsSatisfiednewFormat(ip, tempConstraint)) {
                        ...
                        System.out.println("ERROR: Constraints not satisfied FOR CFIPS \n");
                        System.out.println("Query: "+tempConstraint);
                    }
                }
                if (attainedTimeout) {
                    break;
    ...
    public void evaluateCHFIPSNumberOfDraws(int seed, Database db, String outputfile, int numberOfPatterns, int repetitions, ArrayList<ConstraintNewList.Constraint> query) {

        ArrayList<HashMap<String, Object>> resultsList = new ArrayList<>();
        for(int c = 1; c<= query.size(); c++ ) {    // constraints form 1 to n
            boolean attainedTimeout = false;
            ArrayList<ConstraintNewList.Constraint> tempConstraint = new ArrayList<>(query.subList(0, c));
            CHFIPS chfips = new CHFIPS(seed,db, tempConstraint);
            double GlobalmeanFrequency =0.0;
            double meanDraws = 0.0;
            long startTime = System.currentTimeMillis();
            long timeout = 120_000; // 2 minutes
    ...
    public void evaluateCFIPSNumberOfDraws(Database db, String outputfile, int numberOfPatterns, int repetitions, ArrayList<Constraint> query) {
            //todo: add mean frequency
        ArrayList<HashMap<String, Object>> resultsList = new ArrayList<>();
        for(int c = 1; c<= query.size(); c++ ) {    // constraints form 1 to n
            boolean attainedTimeout = false;
            ArrayList<Constraint> tempConstraint = new ArrayList<>(query.subList(0, c));
            CFIPS cfips = new CFIPS(db, tempConstraint);
            double GlobalmeanFrequency =0.0;
            double meanDraws = 0.0;
            long startTime = System.currentTimeMillis();
            long timeout = 120_000; // 2 minutes
            ...
            for (int iteration = 0; iteration < repetitions; iteration++) {
                ...
                while (patternCount < numberOfPatterns) {
                    ...
                    // Check constraints satisfaction
                    if (ConstraintsSatisfied(ip, tempConstraint)) {
                        patternCount++;
                        meanFrequency += ip.getFrequency();
                    }else{
                        System.out.println("ERROR: Constraints not satisfied FOR CFIPS \n");
    ...
    public ArrayList<Constraint> SelectGenerator(Database db, int generatorType, int numberOfConstraints){
        ConstraintGenerator constGen = new ConstraintGenerator(db, numberOfConstraints, generatorType);
        switch (generatorType) {
            case 1: // Totally random constraints
                return constGen.TotalyRandomConstraints(numberOfConstraints);
            case 2: // Ascending order of attribute constraints
                return constGen.RandomConstraintsOrderedAttributes(numberOfConstraints, true);
            case 3: // Descending order of attribute constraints
                return constGen.RandomConstraintsOrderedAttributes(numberOfConstraints, false);
            case 4: // User-defined constraints (placeholder for future logic)
                break;
            default:
    ...
    public boolean ConstraintsSatisfied(IP ip, ArrayList<Constraint> constraintList){
    ...
    public boolean ConstraintsSatisfiednewFormat(IP ip, ArrayList<ConstraintNewList.Constraint> constraintList){
        for (ConstraintNewList.Constraint c : constraintList) { // ici check le respect des contraintes
            if (c instanceof ConstraintNewList.Inclusion) {
                if (!checkInclusionConstraint(c.getMprim(), c.getVprim(), ip)) {
                    return false;
                }

            } else if (c instanceof ConstraintNewList.Exclusion) {
                if (!checkExclusionConstraint(c.getMprim(), c.getVprim(), ip)) {
                    return false;
                }
            } else if (c instanceof ConstraintNewList.Sup) {
                if (!checkSupConstraint(c.getMprim(), c.getVprim(), ip)) {
                    return false;
                }
            } else if (c instanceof ConstraintNewList.Inf) {
                if (!checkInfConstraint(c.getMprim(), c.getVprim(), ip)) {
                    return false;
                }
            } else if (c instanceof ConstraintNewList.SupEq) {
                if (!checkSupEqConstraint(c.getMprim(), c.getVprim(), ip)) {
                    return false;
                }
            } else if (c instanceof ConstraintNewList.InfEq) {
                if (!checkInfEqConstraint(c.getMprim(), c.getVprim(), ip)) {
                    return false;
                }
            }
    ...
    public void drawRandomIPRespectingConstraints(int seed,ArrayList<Constraint> constraintList, int n, int rep, Database db) {
        /*
         * This method have to retreives n patterns throught FIPS that satisfies the constraints.
         * */
        ...
        for (int r = 0; r < rep; r++) {
            ...
            while (i < n) {
                ...
                IP ip = new RandomIPSampling(db, seed).drawIP();
        ...
        System.out.println("Random IP: " + n + " patterns are drawn in " + result + " iterations");
        System.out.println("Gain(%): " + result / n);


    }

    public void drawConstrainedFipsRespectingConstraints(ArrayList<Constraint> constraintList, int n, int rep, Database db) {
        /*
         * This method have to retreives n patterns throught FIPS that satisfies the constraints.
         * */
        ...
        for (int r = 0; r < rep; r++) {
            ...
            while (i < n) {
                ...
                IP ip = new CFIPS(db, constraintList).drawIP();
        ...
        double result = meanDraws / rep;
        System.out.println("Constrained FIPS: " + n + " patterns are drawn in " + result + " iterations");
        System.out.println("Gain(%): " + result / n);
    }

    public void drawFipsRespectingConstraints(int seed,ArrayList<Constraint> constraintList, int n, int rep, Database db) {
        /*
         * This method have to retreives n patterns throught FIPS that satisfies the constraints.
         * */
        double meanDraws = 0.0;
        ...
        double result = meanDraws / rep;
        System.out.println("FIPS: " + n + " patterns are drawn in " + meanDraws / rep + " iterations");
        System.out.println("Gain (%): " + result / n);
    }
    ...
    public void constrainedPlausibilityRandomIP(int seed,int n, int k, Database realDb, float minfreq, float maxfreq) {
        ...
        while (patternPool.size() < n) {
            ...
            IP ip = new RandomIPSampling(realDb, seed).drawIP();
            float freq = (float) ip.getCoverage().size() / realDb.objectNumber;
            if (freq > minfreq && freq < maxfreq) {
                patternPool.add(ip);
                avgFrequency += freq;
 ...
 /*   public double PlausibilityDensityRandomIP(int n, int k, Database realDb) {
   ...
   /* public double PlausibilityDensityIP(int n, int k, Database realDb) {
    ...
    public void TempsCPURandomIP(int seed,int k, Database db) {
         ...
         * Params: K=> number of patterns; db=> given database
         * */
        RandomIPSampling sampling = new RandomIPSampling(db, seed);
        long startTime = System.nanoTime();
        for (int i = 0; i < k; i++) {
            IP ip = sampling.drawIP();
        }
    ...
    public void evolutionTempsCPURandomIPRejet(int seed,int k, ArrayList<Constraint> constraintList, Database db, String output) {
        /*
         * Method that processes the CPU time evolution of sampling k patterns with random method
         */
        ...
        try (PrintWriter writer = new PrintWriter(new FileWriter(output, true))) {
            ...
            while (i < k) {
                ...
                for (Constraint c : constraintList) { // Check constraints
                    if (c instanceof Inclusion) {
                        if (!checkInclusionConstraint(c.getMprim(), c.getVprim(), ip)) {
                ...
                if (respectConstraint) {
                    if (iterationStartTime == Long.MIN_VALUE) {
                        System.out.println("ERROR: iterationStartTime == -infinity");
                        return;
                    }
                    ...
                    if (iterationElapsedTimeInMs > 60_000) {
                        // If iteration time exceeds 1 minute, log ">1min" and skip to next iteration
                        writer.println((i + 1) + ",>1min");
                    } else {
                        // Write the actual time to CSV if within acceptable limits
    ...
    public void evolutionTempsCPUConstrainedIP(int k, ArrayList<Constraint> constraintList, Database db, String output) {
        /*
        ...
        CFIPS sampling = new CFIPS(db, constraintList);
    ...
    public void evolutionTempsCPUIPRejet(int seed, int k, ArrayList<Constraint> constraintList, Database db, String output) {
        /*
         * Method that process the cpu time evolution of sampling k patterns with FIPS method
         *
         * */
...
/*    public void evolutionFrequenceRandomHIPS(int k,int rep, Database db, String output) {
   ...
   /* public void evolutionFrequenceIP(int k,int rep ,Database db, String output) {
    ...
    public void jaccardCDFRandomIP(int seed,int k, Database db, String outputFilePath) {
        ...
        // 1- Draw k patterns
        RandomIPSampling ips = new RandomIPSampling(db, seed);
        for (int i = 0; i < k; i++) {
            patterns.add(ips.drawIP());
        }
        // 2- Calculate the CDF
 ...
 /*   public double jaccard(Itemset a, Itemset b) {
   ...
   public double fipsnombreMotifavecAttnonContraint(Database db, int seed, int nbmotifs, int repetitions){
    ...
    for (int j =0; j < repetitions;j++) {
        ...
        for (int i = 0; i < nbmotifs; i++) {
            IP ip = fips.drawIP();
            //System.out.println("display IP: "+ip.getIP());
            for (int m=0; m < db.getColumnsNumberNumerical(); m=m+2){
                if(ip.getIP().get(m).equals(db.getDistinctValues().get(m).get(0)) || ip.getIP().get(m+1).equals(db.getDistinctValues().get(m).get(db.getDistinctValues().get(m).size()-1))){
                   results +=1;
                    break;
                }
            }
       ...
       return (Finalresult / repetitions)/nbmotifs;
   }



    public double hfipsnombreMotifavecAttnonContraint(Database db, int seed, int nbmotifs, int repetitions){
        double Finalresult =0;
        HFIPS hfips = new HFIPS(db,seed);
    ...
    public double aleanombreMotifavecAttnonContraint(Database db, int seed, int nbmotifs, int repetitions){
        double Finalresult =0;
        RandomIPSampling random = new RandomIPSampling(db,seed);
